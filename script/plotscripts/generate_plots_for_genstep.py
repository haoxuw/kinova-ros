...
for i in reversed(range(legends.shape[0])):
    ax.plot(data[i][0], data[i][C], label = ("Generator Steps %d Train" % gensteps[i]))
...
C += 1
for i in reversed(range(legends.shape[0])):
    ax.plot(data[i][0], data[i][C], label = ("Generator Steps %d Eval" % gensteps[i]))

save_fname = 'genstep.png'
plt.legend(loc='best')
