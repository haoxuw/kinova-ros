...
gensteps = [1,2,4,8]
for i in reversed(range(legends.shape[0])):
    ax.plot(data[i][0], data[i][C], label = ("Discriminator Steps %d Train" % gensteps[i]))
...
C += 1
for i in reversed(range(legends.shape[0])):
    ax.plot(data[i][0], data[i][C], label = ("Discriminator Steps %d Eval" % gensteps[i]))

save_fname = 'disstep.png'
plt.legend(loc='best')
